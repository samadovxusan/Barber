    ...
    : ControllerBase
{
    ...
    {
        var result = await mediator.Send(bookingGetQuery, cancellationToken);
        return Ok(result);
    }
    ...
    {
        var result = await mediator.Send(new GetByIdBookingQuery() { Id = clientId }, cancellationToken);
        return Ok(result);
    }
    ...
    {
        var result = await mediator.Send(new BookingDeleteCommand() { Id = clientId }, cancellationToken);
